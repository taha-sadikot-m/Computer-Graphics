# Create a Program to perform 3D translation of a square
# Square size and translation input will be taken from user
...
def translate_square(vertices, tx, ty, tz):
    """
    Performs 3D translation on cube vertices.
    
    Args:
        vertices (numpy.ndarray): Original vertices of the cube.
        tx (float): Translation distance along X-axis.
        ty (float): Translation distance along Y-axis.
        tz (float): Translation distance along Z-axis.
    
    Returns:
        numpy.ndarray: Translated vertices.
    """
    translation_vector = np.array([tx, ty, tz])
    translated_vertices = vertices + translation_vector
    return translated_vertices

def plot_square(vertices, color='lightblue', alpha=0.6, edge_color='blue', title="3D Cube"):
    """
...
def plot_comparison(original_vertices, translated_vertices):
    """
    Plots both original and translated cubes for comparison.
    
    Args:
        original_vertices (numpy.ndarray): Original cube vertices.
        translated_vertices (numpy.ndarray): Translated cube vertices.
    """
    ...
    # Translated cube (more prominent)
    translated_faces = [translated_vertices[face_idx] for face_idx in faces_indices]
    translated_collection = Poly3DCollection(
        translated_faces, alpha=0.7, facecolors='lightcoral', 
        edgecolors='red', linewidths=2, label='Translated'
    )
    ax.add_collection3d(translated_collection)
    
    # Plot vertices
    ax.scatter(original_vertices[:, 0], original_vertices[:, 1], original_vertices[:, 2], 
              color='blue', s=80, label='Original Vertices')
    ax.scatter(translated_vertices[:, 0], translated_vertices[:, 1], translated_vertices[:, 2], 
              color='red', s=100, label='Translated Vertices')
    
    # Set axis limits to show both cubes
    all_vertices = np.vstack((original_vertices, translated_vertices))
    ...
    ax.set_ylabel('Y-axis')
    ax.set_zlabel('Z-axis')
    ax.set_title('3D Translation Comparison: Original (Blue) vs Translated (Red)')
    ax.legend()
...
def main():
    """
    Main function to execute the 3D translation program.
    """
    print("=== 3D Cube Translation Program ===")
    print()
    ...
    try:
        ...
        plot_square(original_vertices, title="Original 3D Cube")
        
        # Interactive translation menu
        while True:
            print("\n=== Translation Menu ===")
            print("1. Perform Translation")
            ...
            try:
                ...
                if choice == 1:
                    ...
                    # Perform translation
                    translated_vertices = translate_square(original_vertices, tx, ty, tz)
                    
                    # Display results
                    print(f"\nTranslation applied: ({tx}, {ty}, {tz})")
                    print("Displaying comparison...")
                    
                    # Show both original and translated squares
                    plot_comparison(original_vertices, translated_vertices)
                    
                    # Show translated cube details
                    print("\nTranslated cube vertices:")
